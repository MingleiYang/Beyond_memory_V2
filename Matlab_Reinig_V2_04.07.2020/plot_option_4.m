function [One_diffBm2,meanLRBm,LRmeanBm0,singlevalBm] = ...
    ...
    Cp_eser1,Cp_eser2,Cp_eser3,Ce_pser2,Ce_pser3,n,V,C,BIG_meanB,BIG_meanA,numstep,numstepAfterhs,note)

% plot_option_4  parameter space for memory of activation in embryo, used in Figure S1 and S3 of Reinig et al.
...
elseif parameter1 == mser
    parameter1_label = 'parameter 1: m (PRE size)';
    X2 = 'parameter 1: m';
    name1 = 'm';
    
elseif parameter1 == tfser
    parameter1_label = 'parameter 1: tf (enhancer binding sites)';
    X2 = 'parameter 1: tf; ';
    name1 = 'tf';
    
elseif parameter1 == Cp_eser1
    parameter1_label = 'parameter 1: Cp_e1 (PRE to enhancer)';
    X2 = 'parameter 1: Cp_e1; ';
    name1 = 'Cp_e1';
    
elseif parameter1 == Cp_eser2
    parameter1_label = 'parameter 1: Cp_e2 (PRE to enhancer)';
    X2 = 'parameter 1: Cp_e2; ';
    name1 = 'Cp_e2';
    
elseif parameter1 == Cp_eser3
    parameter1_label = 'parameter 1: Cp_e3 (PRE to enhancer)';
    X2 = 'parameter 1: Cp_e3; ';
    name1 = 'Cp_e3';
    
elseif parameter1 == Ce_pser2
    parameter1_label = 'parameter 1: Ce_p2 (enhancer to PRE)';
    X2 = '(parameter 1: Ce_p2; ';
    name1 = 'Ce_p2';

elseif parameter1 == Ce_pser3
    parameter1_label = 'parameter 1: Ce_p3 (enhancer to PRE)';
...
elseif parameter2 == mser
    parameter2_label = 'parameter 2: m (PRE size)';
    X1 = 'parameter 2: m';
    name2 = 'm';

elseif parameter2 == tfser
    parameter2_label = 'parameter 2: tf (enhancer binding sites)';
    X1 = 'parameter 2: tf; ';
    name2 = 'tf';
    
elseif parameter2 == Cp_eser1
    parameter2_label = 'parameter 2: Cp_e1 (PRE to enhancer)';
    X1 = 'parameter 2: Cp_e1; ';
    name2 = 'Cp_e1';
    
elseif parameter2 == Cp_eser2
    parameter2_label = 'parameter 2: Cp_e2 (PRE to enhancer)';
    X1 = 'parameter 2: Cp_e2; ';
    name2 = 'Cp_e2';
    
elseif parameter2 == Cp_eser3
    parameter2_label = 'parameter 2: Cp_e3 (PRE to enhancer)';
    X1 = 'parameter 2: Cp_e3; ';
    name2 = 'Cp_e3';
    
elseif parameter2 == Ce_pser2
    parameter2_label = 'parameter 2: Ce_p2 (enhancer to PRE)';
    X1 = '(parameter 2: Ce_p2; ';
    name2 = 'Ce_p2';

elseif parameter2 == Ce_pser3
    parameter2_label = 'parameter 2: Ce_p3 (enhancer to PRE)';
...
for cell = 1:n % need to recheck this as well. 06.08.18
    
embryoB1(cell,:) = BIG_meanB (cell*numstep-numstepAfterhs,:);%extract the last window of the heatshock in segment 2.
embryoB2(cell,:) = BIG_meanB (cell*numstep,:);%extract the last window of the memory phase

embryoA1(cell,:) = BIG_meanA (cell*numstep-numstepAfterhs,:);%%extract the last window of the heatshock in segment 2.
embryoA2(cell,:) = BIG_meanA (cell*numstep,:);%extract the last window of the memory phase

end

controlBe = embryoB1(:,1:2); %extracts the first column of the enhancer without coupling for heatshock, these are the control values
controlB1 = controlBe; 


for x = 1:(V*C/2)-1 
    
controlB1= [controlB1,controlBe]; % concatenates repeats of the first two columns to a matrix of dimensions same as embryo B1.  

end  

% control B1 is the enhancer in early embryo without coupling
% calculate differences between experiment and control within each
% simulation.

diffBe = embryoB1 - controlB1;   %calculates the difference between enhancer in heatshock phase and control.
diffBm = embryoB2 - controlB1;   %calculates the difference between enhancer at end of maintenance phase and control.  
diffAe = embryoA1 - controlB1;   %calculates the difference between PRE in heatshock phase and control.
diffAm = embryoA2 - controlB1;   %calculates the difference between PRE at end of maintenance phase and control.  
...
% calculate the mean and std of the enhancer values in the last window of
...
for i = 1: V*C/2
    ...
    meanLBe (i) = meandiffBe (2*i-1); % extract the left values for the enhancer
    meanRBe (i) = meandiffBe (2*i); % extract the right values for the enhancer 
    meanLRBe (i) = (meandiffBe (2*i-1) + meandiffBe (2*i) )/2; % calculate the mean of each pair of left and right columns (enhancer)
    
    meanLBm (i) = meandiffBm (2*i-1); % extract the left values for the enhancer
    meanRBm (i) = meandiffBm (2*i); % extract the right values for the enhancer 
    meanLRBm (i) = (meandiffBm (2*i-1) + meandiffBm (2*i) )/2; % calculate the mean of each pair of left and right columns (enhancer)
    
    meanLAe (i) = meandiffAe (2*i-1); % extract the left values for the enhancer
    meanRAe (i) = meandiffAe (2*i); % extract the right values for the enhancer 
    meanLRAe (i) = (meandiffAe (2*i-1) + meandiffAe (2*i) )/2; % calculate the mean of each pair of left and right columns (PRE)
    
    meanLAm (i) = meandiffAm (2*i-1); % extract the left values for the enhancer
    meanRAm (i) = meandiffAm (2*i); % extract the right values for the enhancer 
    meanLRAm (i) = (meandiffAm (2*i-1) + meandiffAm (2*i) )/2; % calculate the mean of each pair of left and right columns (PRE)
    
    mean_OneLRBm (i) = (meanOne_diffBm2 (2*i-1) + meanOne_diffBm2 (2*i) )/2; % calculate the mean of each pair of left and right columns (PRE)
    std_OneLRBm (i) = (stdOne_diffBm2 (2*i-1) + stdOne_diffBm2 (2*i) )/2; % calculate the mean of each pair of left and right columns (PRE)
    ...
    % extract the left and rigt values of the mean and std enhancer levels at the end of
    % the maintenance phase. 
   
    meanLembryoB2 (i) = meanembryoB2 (2*i-1); % extract the left mean values for the enhancer
    meanRembryoB2 (i) = meanembryoB2 (2*i); % extract the right mean values for the enhancer
    stdLembryoB2 (i)  = stdembryoB2 (2*i-1); % extract the left std for the enhancer
    stdRembryoB2 (i)  = stdembryoB2 (2*i); % extract the right std for the enhancer
    
    % Make separate matrices for left and right (used for A and P in plot option 1 and 2).
   ...
   % Make single matrix for mean of left and right (used for plot option 4).
   
   meanLRembryoB2 (i) = (meanembryoB2 (2*i-1) + meanembryoB2 (2*i) )/2;
   stdLRembryoB2 (i) = (stdembryoB2 (2*i-1) + stdembryoB2 (2*i) )/2;
   
    meanstdLRembryoB2 = [meanLRembryoB2;stdLRembryoB2];% this makes a two row matrix with mean memory on top and Stddev below. 
...
leftBe = reshape (meanLBe, numR,numC); %left side for enhancer
rightBe = reshape (meanRBe, numR,numC); %right side for enhancer
LRmeanBe = reshape (meanLRBe, numR,numC);%mean of left and right for enhancer

leftBm = reshape (meanLBm, numR,numC); %left side for enhancer
rightBm = reshape (meanRBm, numR,numC); %right side for enhancer
LRmeanBm = reshape (meanLRBm, numR,numC);%mean of left and right for enhancer

leftAe = reshape (meanLAe, numR,numC); %left side for enhancer
rightAe = reshape (meanRAe, numR,numC); %right side for enhancer
LRmeanAe = reshape (meanLRAe, numR,numC);%mean of left and right for enhancer

leftAm = reshape (meanLAm, numR,numC); %left side for enhancer
rightAm = reshape (meanRAm, numR,numC); %right side for enhancer
LRmeanAm = reshape (meanLRAm, numR,numC);%mean of left and right for enhancer
...
colormap (flipud (parula));
 ...
 title1 = {'Plot option 4, parameter space: activation';note;'';'promoter memory'};
 title2 = {'Plot option 4, parameter space: activation';note;'';'promoter heatshock: all data'};
 


 subplot1 = subplot(1,2,1);%late embryo enhancer compared to control
 pcolor (LRmeanBm0); caxis ([0.03,0.27]);shading interp;
 title (title1);
 ...
 subplot2 = subplot(1,2,2);%late embryo PRE compared to control
 pcolor (LRmeanAm0); caxis ([0.03,0.27]);shading interp;
...
subplot3 = subplot(2,2,1); 
 ...
 title ('PRE/TRE heatshock: all data');
 set(gca,'XTick',1:num1*2:(num2*num1*2)+num1*2);
 set(gca,'XTickLabel',(parameter2_list));
 xlabel(Xlabel2);
 ylabel('repetitions');
...
title3 = {'Plot option 4: parameter space: activation';note;'';'Promoter memory: means (1= good, 0.4 = bad)'};

subplot7 = subplot(2,1,1);
hold on
ax1 = subplot(2,1,1);
bar(singlevalBm (1,:))
...
title (title3);
 ...
 set(gca,'XTickLabel',(parameter2_list));
 xlabel(Xlabel2);
 ylabel('Activation memory');
 
subplot8 = subplot(2,1,2);
hold on
ax2 = subplot(2,1,2);
bar(meanstdLRembryoB2 (1,:))
errorbar(meanstdLRembryoB2 (1,:),meanstdLRembryoB2 (2,:),'.k');
ylim(ax2,[0 1.6])
title ('Promoter levels at end of maintenance phase: should be active if early heatshock');
 set(gca,'XTick',1:num1:(num2*num1)+num1);
 set(gca,'XTickLabel',(parameter2_list));
 xlabel(Xlabel2);
 ylabel('N_B/s');
