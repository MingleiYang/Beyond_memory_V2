function [nucs, sites, OUT,UPHILLOUT] = timecourse_functionUPHILL (C_up,h_up,ti,m,tf,model,p3,p4,p5,p6,Cp_e,Ce_p,p1,p2,nucstart,sitestart,couple_ep_p3start,couple_ep_p4start)
  
% timecourse_functionUPHILL   simulates PRE/TRE and promoter over time t.
% Leonie Ringrose, 17.05.19
% Specialised to adjust p1 via Hill function for eya gradient activation in eye disc
...
for i= 1:ti
    ...
    %attempt noisy conversion on the modified or unmodified nucleosome "b" 
    %----------------------------------------------------------------------------------------------------
    if (r5<=p5) % probability of noisy conversion
          ...
          else % if at 0, go up or down with probability p6 (=0.5).
   ...
   % Calculate the UPHILL function: 
   
   
 
    Hill = (C_up*(i/600)^h_up/(1+(i/600)^h_up))+0.1;
    UPHILLOUT (i) = Hill;
