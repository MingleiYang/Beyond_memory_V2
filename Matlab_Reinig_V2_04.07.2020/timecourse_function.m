 function [nucs, sites, OUT] = timecourse_function (ti,m,tf,model,p3,p4,p5,p6,Cp_e,Ce_p,p1,p2,nucstart,sitestart,couple_ep_p3start,couple_ep_p4start)
  
% timecourse_function   simulates PRE/TRE and promoter over time t.
...
for i= 1:ti
    ...
    %attempt noisy conversion on the previously modified or unmodified nucleosome 
    %----------------------------------------------------------------------------------------------------
    if (r5<=p5) % probability of noisy conversion
          ...
          else % if at 0, go up or down with probability p6.
    ...
    p1co=p1*couple_p1; % use for the PRE affecting the enhancer. 
...
if sites(i,z)==0
    ...
    if r1<=p1co %attempt binding at s1 with probability p1co.
       sites (i+1,z) = 1; % if 0 it goes to 1 or stays 1 if it is already 1
    else
       sites(i+1,z) = sites(i,z);% if 0 it stays 0, if 1 its stays 1
    end
        ...
        sites(i+1,z)= sites(i,z); % if r2 > p2, no change, it stays bound or unbound
        end                
end        
